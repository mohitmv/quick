...
// testing ts::Hash for custom defined types exposing GetHash method.
  ...
  struct Company {
    struct Release {
      ...
      size_t GetHash() const {
        return ts::HashF(make_tuple(id, release_date, bugs, release_tags));
      }
    };
    int num_members = 0;
    ...
    size_t GetHash() const {
      return ts::HashF(make_tuple(num_members, releases, task_pairs));
    }
  };
  ts::Hash<Company> hash_f3;
  ts::Hash<Company::Release> hash_f4;
