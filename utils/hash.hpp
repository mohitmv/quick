...
// ts::Hash Complete Reference : http://bit.ly/2YzejTs
// Implements ts::Hash by extending std::hash. Supports hashing for complex
// types.
// Sample usage:
// ts::Hash<pair<int, map<int, string>>> hasher;
// using ComplexType = vector<pair<string, vector<int>>>;
// ts::Hash<tuple<int, string, set<ValueTypeEnum>, ComplexType>>;
// struct ComplexClass {
//   ....
...
//   };
// };
// ts::Hash<pair<ComplexType, vector<ComplexClass>>> hasher2;
...
#include "ts/type_traits.hpp"

namespace ts {

// `ts::Hash` is ThoughtSpot specfic hash, derived from std::hash.
...
std::size_t OrderedSequenceHash(const Container& input) {
  ...
  auto hasher = ts::Hash<typename Container::value_type>();
  int pointer = 0;
  for (auto& e: input) {
...
std::size_t OrderedMapHash(const MapContainer& input) {
  ...
  auto key_hasher = ts::Hash<typename MapContainer::key_type>();
  auto value_hasher = ts::Hash<typename MapContainer::mapped_type>();
  int pointer = 0;
  for (auto& e: input) {
...
std::size_t PairHash(const std::pair<T1, T2>& p) {
  std::vector<std::size_t> v = {ts::Hash<T1>()(p.first),
                                ts::Hash<T2>()(p.second)};
...
std::size_t TupleHashImplHelper(const std::tuple<Ts...> &input,
  ...
  std::vector<std::size_t> hashed_elements = {
    ts::Hash<typename std::tuple_element<index, std::tuple<Ts...>>::type>()(
      std::get<index>(input))...
  };
...
//   ....
//   std::size_t GetHash() const {
//      return ts::HashF(make_tuple(t1, t2, t2));
//   }
// };
// And then define the GetHash for T1, T2, and T3 recursively.
template<typename T>
struct Hash<T, ts::void_t<decltype(&T::GetHash)>> {
  std::size_t operator()(const T& t) const {
    return t.GetHash();
  }
};
